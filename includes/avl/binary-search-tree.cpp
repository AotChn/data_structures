...
std::string in_order_string(tree_node<T>* root){
    ...
    if(root == nullptr){}
        ...
        ss<<in_order_string(root->_left);
        ss<<"["<< root->_item <<"]";
        ss<<in_order_string(root->_right);
    }
    ss >> s;
    return s;
}
...
bool avl_tree_erase(tree_node<T>*& root, const T& target){
    ...
    //search for node 
    if(root->_item > target){
        bool l = avl_tree_erase(root->_left, target);
        root->update_height();
        root = rotate(root);
    }
    else if(root->_item < target){
        bool r = avl_tree_erase(root->_right, target);
        root->update_height();
        root = rotate(root);
    }
    else{
...
template <typename T>
std::string tree_str(tree_node<T> * root){
    std::stringstream ss; 
    std::string s;
    ss << root->_item;
    ss >> s;
...
inline int find_space(int n, int max){ //must be iterative recursive function
    int k=0;
    for(int i=0; i<n; i++){
        k = k*2+max;
    }
...
inline void print_space(std::ostream& outs, int n){ 
    for(int i=0; i<find_space(n, MAX_CHAR);i++){
        outs<< " ";
    }
}
template <typename T>
struct nodep{ //stores tree_node w/ level node is in relation to tree(from true root)
    nodep(){}
    nodep(tree_node<T>* d, int parent_height) : _data(d), _pH(parent_height){}
...
template <typename T>
inline void process_root(std::ostream& outs, Queue<nodep<T>*>& b, int& height){
    if(height != b.front()->_pH){ //change in level 
        height--;
        outs<<'\n';
        print_space(outs, height);
    }
    else
        print_space(outs, height+1);

    if((b.front()->_data != nullptr)){
        nodep<T>* l = new nodep<T>(b.front()->_data->_left,height-1);
        b.push(l);
    }
    if((b.front()->_data != nullptr)){
        nodep<T>* r = new nodep<T>(b.front()->_data->_right,height-1);
        b.push(r);
    }
    if(b.front()->_data == nullptr){
        outs << std::setw(MAX_CHAR)<<"N";
    }   
    else
        outs << std::setw(MAX_CHAR)<<b.front()->_data->_item<<"";
...
template <typename T>
void BetterTreePrint(tree_node<T> *root, std::ostream& outs){
    nodep<T>* hold = new nodep<T> (root, root->_height);
    int height = root->_height+2; //ensures that we have at least 1 space at root level *
    int n = find_space(height, MAX_CHAR); //max number of nodes possible to be visited
    Queue<nodep<T>*> B;
    B.push(hold);
    while(!B.empty() && n>0){
        process_root(outs, B, height);
        n--;
    }
}

//* for the algorithm to work we must initially set the height of the tree to root height +1 and artificually trigger a level change
// to ensure we dont give an additional endl in the start 
//we then have to add an additional height to ensure when we reach the floor of the tree we will have at least 1 space or else the values 
//will be scrunched up next to each other 
// we get this because k0 = 0 and k1 = 0*2+1 = 1 so by adding an additional height we ensure the floor will k = 1

/*
    WHY DOES THE MAX_CHAR matter ?
        - we are evenly spacing each value by a dx amount meaning it must match the spacing of the max char value or else we will get an uneven spacing 
        i.e. 2 =/= 220 so we must do --2 to match 220
        in this case we set each unit dx to the max of 3 
        since this difference is only changing by a constant we can just change our algorithm by a constant 
